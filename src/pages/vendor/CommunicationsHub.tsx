...
import {
  ...
  CheckCheck, Phone, Filter, Archive, ArrowLeft
} from 'lucide-react';
                    ...
                    return (
                      ...
                      >
                        <div className={`max-w-[70%] ${isVendor ? 'order-2' : 'order-1'}`}>
                          ...
                          <div className={`flex items-center gap-1 mt-1 text-xs text-gray-500 ${isVendor ? 'justify-end' : 'justify-start'}`}>
                            <Clock className="h-3 w-3" />
                            {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            {isVendor && msg.is_read && <CheckCheck className="h-3 w-3" />}
                          </div>
                        </div>
                      </div>
                    );
